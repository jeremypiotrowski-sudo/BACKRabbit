...
public readonly struct SaharaPacket
{
    ...
    public const int MaxPacketSize = 0x40000;
    ...
    public static SaharaPacket Parse(ReadOnlySpan<byte> data)
    {
        if (data.Length < HeaderSize)
            throw new SaharaProtocolException($"Packet too short: {data.Length} bytes");
    ...
    public override string ToString() => $"SaharaPacket(Cmd=0x{Command:X2}, Len={Length})";
}
